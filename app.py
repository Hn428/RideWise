...
def get_weather_background_css(hour, weather_type):
    """
    Generate a CSS gradient background based on time of day and weather
    ...
    Returns:
        CSS string for background gradient
    """
    # Time of day colors
    if 5 <= hour < 8:  # Dawn
        bg_colors = ["#F9CDAD", "#FFA26B"]
    elif 8 <= hour < 16:  # Day
        bg_colors = ["#87CEEB", "#4682B4"]
    elif 16 <= hour < 19:  # Dusk
        bg_colors = ["#FFB85F", "#D19275"]
    else:  # Night
        bg_colors = ["#1c2951", "#111827"]
    
    # Modify colors based on weather
    if weather_type == 'rain' or weather_type == 'sleet':
        bg_colors = ["#6b7280", "#374151"]
    elif weather_type == 'thunderstorm':
        bg_colors = ["#1f2937", "#111827"]
    elif weather_type == 'snow':
        bg_colors = ["#e5e7eb", "#d1d5db"]
    elif weather_type == 'fog' or weather_type == 'mist':
        bg_colors = ["#9ca3af", "#6b7280"]
    
    # Create CSS for linear gradient
    css = f"""
    <style>
    .stApp {{
...
from model import SurgePredictor
...
def load_model():
    ...
    model_path = os.path.join('models', 'surge_model.joblib')
...
def display_prediction_tab():
    ...
    if model is None:
        st.warning("⚠️ No trained model found. Please go to the Training tab to train a model first.")
        return
    ...
    try:
        ...
        # Display the map with both locations
        st.map(map_data, zoom=11) # Zoom out slightly to see both points
    ...
    with time_weather_col1:
        ...
        # Time details
        now = datetime.now()
        date_input = st.date_input("Date", value=now.date())
        time_input = st.time_input("Time", value=now.time())
        
        # Combine date and time into a datetime object
        timestamp = datetime.combine(date_input, time_input)
        
        # Store selected time in session state to preserve it
        if timestamp != st.session_state.get('selected_timestamp', None):
    ...
    with time_weather_col2:
        ...
        use_real_weather = st.checkbox("Use real-time weather data", value=True)
    ...
    """, unsafe_allow_html=True)
        ...
        if not use_real_weather:
            temperature = st.slider("Temperature (°C)", min_value=-10.0, max_value=40.0, value=20.0, step=0.5)
            humidity = st.slider("Humidity (%)", min_value=0.0, max_value=100.0, value=50.0, step=5.0)
            wind_speed = st.slider("Wind Speed (km/h)", min_value=0.0, max_value=50.0, value=10.0, step=1.0)
    ...
    if predict_button:
        with st.spinner("Calculating surge pricing..."):
            try:
                ...
                st.markdown("<div style='background-color: rgba(76, 175, 80, 0.1); padding: 20px; border-radius: 10px; margin-top: 20px;'>", unsafe_allow_html=True)
                ...
                if result['surge_multiplier'] > 1.5:
                    st.info("⚠️ Surge pricing is high! Consider waiting or choosing a different time.")
        ...
        if prediction_successful:
             ...
             if best_time is not None:
                 st.markdown("<div style='background-color: rgba(52, 152, 219, 0.1); padding: 20px; border-radius: 10px; margin-top: 20px;'>", unsafe_allow_html=True)
...
def display_training_tab():
    ...
    if train_button:
        ...
        with st.spinner("Training model... This may take a few minutes."):
            ...
            # Save model
            model.save_model('models/surge_model.joblib')
